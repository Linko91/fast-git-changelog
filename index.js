	...
	var child = exec(command, function(error, stdout, stderr) {
	    ...
	    if(stderr){
		    ...
		    fs.writeFile(cl_path, parseLog(stdout), function(err) {
		    	spin.stop();
			    console.log(err || '\n'+chalk.green.bold('\u2714 ')+chalk.white.bold('CHANGELOG created on ')+chalk.magenta(cl_path)+'\n');
			});
	    }
	});
}
...
function parseLog(raw) {
	...
	for(var k=0; k<commits.length; k++){
		...
		if(commits[k].data.tags){
			tmpmd = "\n`"+commits[k].data.tags+"` "+moment(commits[k].data.date, 'YYYY-MM-DD HH:mm:ss ZZ').format('DD/MM/YYYY HH:mm')+"\n----\n";
		}
